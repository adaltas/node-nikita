...
module.exports = function() {
  var call_async, call_callback, call_sync, handler, j, jump_to_error, len, name, obj, option, properties, proto, read_options, ref, run, stack, todos;
  ...
  read_options = function(action) {
    var arg, callback, global_options, i, k, kv, l, len1, len2, len3, len4, len5, local_options, local_options_array, local_opts, m, n, o, options, opts, p, parent_options, ref1, v;
    ...
    for (l = 0, len1 = ref1.length; l < len1; l++) {
      ...
      if (typeof arg === 'function') {
        ...
        for (i = m = 0, len2 = arg.length; m < len2; i = ++m) {
          kv = arg[i];
          if (local_options[i] == null) {
            local_options[i] = {};
          }
          for (k in kv) {
            v = kv[k];
    ...
    options = [];
    for (n = 0, len3 = local_options.length; n < len3; n++) {
      local_opts = local_options[n];
  ...
  call_sync = function(action) {
    var callback, err, mtodos, options, ref1, status;
    ref1 = read_options(action), options = ref1[0], callback = ref1[1];
  ...
  call_async = function(action) {
    var callback, err, finish, options, ref1, status_action, status_callback;
    ref1 = read_options(action), options = ref1[0], callback = ref1[1];
    try {
      todos.status.unshift(void 0);
      stack.unshift(todos);
  ...
  run = function() {
    var arg, args, err, fn, handler, ref1, status, todo;
    todo = todos.shift();
    if (!todo) {
    ...
    if (todo.type === 'call') {
      ...
      args = (function() {
        ...
        for (l = 0, len1 = ref1.length; l < len1; l++) {
          arg = ref1[l];
          if (!handler && typeof arg === 'function') {
            handler = arg;
            continue;
          }
          results.push(arg);
