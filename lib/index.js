...
module.exports = function() {
  var afters, befores, call_before, call_callback, enrich_options, handler, j, jump_to_error, len, name, normalize_arguments, obj, option, properties, proto, ref, run, stack, store, todos;
  ...
  normalize_arguments = function(_arguments, type) {
    var arg, callback, handler, i, k, l, len1, len2, len3, len4, len5, len6, m, multiple, n, o, options, opts, p, q, v;
    if (type == null) {
      type = 'call';
    }
    multiple = false;
  ...
  call_before = function(action, callback) {
    return each(befores).run(function(before, next) {
      if (typeof before.target === 'string') {
        before.target = {
          type: before.target
        };
      }
      if (action.type !== before.target.type) {
        return next();
      }
      if (before.handler.length === 2) {
        return before.handler.call(obj, action.options, next);
      } else {
  ...
  run = function() {
    var action, callback, err, ref1, status;
    action = todos.shift();
    if (!action) {
    ...
    callback = function(err, throw_error, statuses, user_args) {
      var callback_args, status_callback;
    ...
    return wrap.options(action.options, function(err) {
      var options, statuses, user_args;
      statuses = [];
      user_args = (function() {
      ...
      return call_before(action, function(err) {
        var throw_error;
        if (err) {
          return next(err);
        }
        ...
        return each(action.options).run(function(options, index, next) {
          ...
          return conditions.all(obj, options, function(err) {
            ...
            try {
              if (options.handler.length === 2) {
                return options.handler.call(obj, options, function() {
          ...
          return callback(err, throw_error, statuses, user_args);
        });
      });
    });
  };
