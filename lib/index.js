...
module.exports = function() {
  var afters, before, befores, call_, call_async, call_callback, call_sync, enrich_options, handler, j, jump_to_error, len, name, normalize_arguments, obj, option, properties, proto, ref, run, stack, todos;
  ...
  normalize_arguments = function(_arguments, type) {
    var arg, callback, handler, i, k, l, len1, len2, len3, len4, m, multiple, n, o, options, v;
    if (type == null) {
      type = 'call';
    }
    multiple = false;
  ...
  call_ = function(action) {
    var callback, err;
    action.options = enrich_options(action.options);
    callback = function(err, statuses, user_args) {
    ...
    try {
      ...
      if (action.handler.length === 2) {
        return call_async(action, callback);
      } else {
        return call_sync(action, callback);
      }
    } catch (_error) {
      err = _error;
      todos = stack.shift();
  ...
  before = function(action, callback) {};
  call_sync = function(action, callback) {
    var options, status;
  ...
  call_async = function(action, callback) {
    return wrap.options(action.options, function(err) {
      var options, status_action, status_callback, statuses, user_args;
      status_callback = [];
      status_action = [];
      statuses = [];
      user_args = (function() {
      ...
      return each(action.options).run(function(options, index, next) {
        ...
        return callback(err, statuses, user_args);
      });
    });
  };
...
(function() {
  ...
  return register('call', module.exports().call, true);
})();
