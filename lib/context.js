...
// # Nikita Context
var EventEmitter, array, conditions, domain, each, handle_get, merge, path, promise, registry, string, todos_create, todos_reset, util, wrap;

module.exports = function() {
  var _run_, call_callback, domain_on_error, enrich_options, handle_multiple_call, jump_to_error, normalize_options, obj, proxy, ref, reg, run, state;
  ...
  obj.internal = {};
  obj.internal.options = function(_arguments, type, params = {}) {
...
// Convert every argument to an array
  ...
  enrich_options = function(user_options) {
    var _logs, k, log_disabled, match, options, parent_options, ref1, ref2, session_options, v;
    user_options.enriched = true;
    session_options = obj.options;
    if (session_options.cascade == null) {
...
// Merge action options with default session options 
    ...
    options.log = function(log) {
      var _log, args, file, frame, header, l, len, len1, line, m, method, ref3, stackTrace;
      ...
      for (m = 0, len1 = _logs.length; m < len1; m++) {
        _log = _logs[m];
        _log(log);
      }
      if (!log_disabled) {
  ...
  jump_to_error = function(err) {
    var ref1;
    while (state.todos[0] && ((ref1 = state.todos[0].type) !== 'catch' && ref1 !== 'next' && ref1 !== 'promise')) {
      state.todos.shift();
    }
  ...
  run = function(options, callback) {
    var err, header, index, k, org_options, parent_options, ref1, ref2, ref3, status, v;
    if (!options) {
    ...
    for (k in parent_options) {
      v = parent_options[k];
      if (org_options[k] === void 0 && k !== 'log' && obj.cascade[k] === true) {
        org_options[k] = v;
      }
    }
    ...
    if (array.compare(options.type, ['end'])) {
      return conditions.all(proxy, options, function() {
        var ref4;
        while (state.todos[0] && ((ref4 = state.todos[0].type) !== 'next' && ref4 !== 'promise')) {
          state.todos.shift();
        }
        if (callback) {
          callback(err);
...
// Remove conditions from options
      ...
      options.attempt = -1;
      do_handler = function() {
        var called, cascade, do_next, handle_async_and_promise, option, options_callback, options_handler, options_handler_length, opts, promise_returned, ref4, ref5, ref6, result, status_sync, wait_children;
        options.attempt++;
        ...
        if (options.handler == null) {
          options.handler = ((ref4 = obj.registry.get(options.type)) != null ? ref4.handler : void 0) || ((ref5 = registry.get(options.type)) != null ? ref5.handler : void 0);
        }
        if (!options.handler) {
