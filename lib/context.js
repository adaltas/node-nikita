...
var EventEmitter, called_deprecate_destination, called_deprecate_local_source, conditions, domain, each, registry, string, todos_create, todos_reset, wrap,
...
module.exports = function() {
  var _run_, afters, befores, call_callback, depth, domain_on_error, enrich_options, handle_multiple_call, headers, j, jump_to_error, killed, len, normalize_options, obj, once, option, properties, proto, proxy, ref, ref1, run, stack, store, todos;
  ...
  proxy = new Proxy(obj, {
    has: function(target, name) {
      return target[name] || registry[name];
    },
    get: function(target, name) {
      ...
      return function() {
        ...
        args.unshift(target.registry[name] || registry[name]);
    ...
    return wrap.options(options, function(err) {
      ...
      do_handler = function() {
        ...
        if (options.handler == null) {
          options.handler = obj.registry[options.type] || registry[options.type];
        }
