...
module.exports = function() {
    ...
    get: function(target, name) {
      ...
      get_proxy_builder = function() {
        ...
        builder = function() {
          ...
          if (todos.length === options.length) { // Activate the pump
            setImmediate(_run_);
          }
          return proxy;
        };
  ...
  run = function(options, callback) {
    ...
    if (!options) { // Nothing more to do in current queue
      if (callback) {
        callback(todos.err);
      } else {
          ...
          if (options_handler_length === 2) { // Async style
            return options_handler.call(proxy, opts, function() {
              ...
              return setImmediate(function() {
                return do_next(args); // Sync style
              });
            });
          } else {
  ...
  properties.then = {
    get: function() {
      return function() {
        ...
        if (todos.length === 1) { // Activate the pump
          setImmediate(_run_);
        }
        return proxy;
      };
