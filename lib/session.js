...
module.exports = function() {
  ...
  domain_on_error = function(error) {
    error.message = `Invalid State Error [${error.message}]`;
    return handle_multiple_call(error);
  };
...
// Merge action options with default session options 
  ...
  run = function(options, callback) {
    var error, index, k, options_original, options_parent, ref1, ref2, ref3, status, v;
    if (!options) {
    ...
    if (!options) {
      ...
      if (callback) {
        if (callback) {
          callback(state.current_level.error);
        }
        return;
      } else {
