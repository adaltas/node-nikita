...
// Convert every argument to an array
    ...
    actions = (function() {
      ...
      for (p = 0, len4 = actions.length; p < len4; p++) {
        ...
        if (params.enrich && ((ref1 = action.callback) != null ? ref1.length : void 0) > 2) {
        ...
        if (action.status === false && action.shy === false) { // Room for argument, leave it strict for now until we come accross a usecase justifying it.
        ...
        if (!(typeof action.sleep === 'number' && action.sleep >= 0)) {
          jump_to_error(Error(`Invalid options sleep, got ${JSON.stringify(action.sleep)}`));
        }
        results.push(action);
      }
      return results;
...
// Remove conditions from options
      ...
      do_callback = function(args) {
        ...
        state.todos.status[0].value = options.status ? args[1].status : false;
        if (options.callback) {
