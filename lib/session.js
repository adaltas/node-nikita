      ...
      get_proxy_builder = function() {
        ...
        builder = function() {
          ...
          for (l = 0, len = options.length; l < len; l++) {
            opts = options[l];
            state.current_level.push(opts);
          }
          if (state.current_level.length === options.length) { // Activate the pump
            setImmediate(_run_);
          }
          return proxy;
        };
...
// Merge action options with default session options 
  ...
  call_callback = function(fn, args) {
    ...
    state.current_level = state.parent_levels.shift();
    if (mtodos.length) {
      return state.current_level.unshift(...mtodos);
    }
  };
  ...
  jump_to_error = function(err) {
    var ref1;
    while (state.current_level[0] && ((ref1 = state.current_level[0].action) !== 'catch' && ref1 !== 'next' && ref1 !== 'promise')) {
      state.current_level.shift();
    }
  ...
  run = function(options, callback) {
    ...
    if (!options) {
      options = state.current_level.shift();
    }
    ...
    if (array.compare(options.action, ['end'])) {
      return conditions.all(proxy, {
        ...
        var ref4;
        while (state.current_level[0] && ((ref4 = state.current_level[0].action) !== 'next' && ref4 !== 'promise')) {
          state.current_level.shift();
        }
        if (callback) {
          callback(err, {});
...
// Remove conditions from options
  ...
  state.properties.next = {
    get: function() {
      return function() {
        state.current_level.push({
          action: 'next',
          handler: arguments[0]
        });
        if (state.current_level.length === 1) { // Activate the pump
          setImmediate(_run_);
        }
        return proxy;
      };
...
state_reset_level = function(todos) {
  todos.err = null;
  todos.status = [];
