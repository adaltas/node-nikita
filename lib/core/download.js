...
module.exports = function(options) {
  var algo, destination, hash, hash_info, protocols_ftp, protocols_http, ref, ref1, source, stageDestination;
  ...
  this.call({
    ...
    handler: function(_, callback) {
      options.log({
        message: "Check if destination (" + destination + ") exists",
        level: 'DEBUG',
        module: 'mecano/lib/download'
      });
  ...
  this.call(function(_, callback) {
    ...
    } else if (!options.ssh && options.cache) {
      if (hash === true) {
        hash_info = {
          ssh: null,
          source: source
  ...
  this.call(function(_, callback) {
    ...
    options.log({
      message: "Compare the downloaded file with the provided checksum",
      level: 'DEBUG',
      module: 'mecano/lib/download'
    });
    return misc.file.hash(options.ssh, stageDestination, algo, function(err, calc_hash) {
      ...
      if (hash === calc_hash) {
        options.log({
          message: "Mecano `download`: Hash match with staged uploaded file",
          level: 'DEBUG',
          module: 'mecano/lib/download'
        });
        return callback();
      ...
      return misc.file.remove(options.ssh, stageDestination, function(err) {
        ...
        return callback(Error("Invalid checksum, found \"" + calc_hash + "\" instead of \"" + hash + "\""));
      });
    });
  });
