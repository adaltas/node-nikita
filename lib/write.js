...
module.exports = function(goptions, options, callback) {
  ...
  misc.options(options, function(err, options) {
    ...
    return each(options).parallel(goptions.parallel).on('item', function(options, next) {
      var append, between, content, destination, destinationHash, do_backup, do_diff, do_end, do_eof, do_ownership, do_permissions, do_read_destination, do_read_source, do_render, do_replace_partial, do_write, from, modified, to, w, write, _i, _len, _ref1;
      ...
      if ((options.from != null) || (options.to != null) || (options.match != null) || (options.replace != null)) {
        write.push({
          from: options.from,
          to: options.to,
          match: options.match,
      ...
      do_read_source = function() {
        ...
        if (typeof options.log === "function") {
          options.log("Mecano `write`: force local source is \"" + (!options.local_source ? 'not' : void 0) + " active\"");
        }
        if (typeof options.log === "function") {
      ...
      do_replace_partial = function() {
        var linebreak, opts, orgContent, pos, posoffset, res, _j, _len1;
        if (!write.length) {
          return do_eof();
        }
        if (typeof options.log === "function") {
