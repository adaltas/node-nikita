...
module.exports = function(options, callback) {
  ...
  delete options.cmd;
  console.log(cmd, options.container);
    ...
    return docker.exec(" ps -a | grep '" + options.container + "'", options, true, (function(_this) {
      return function(err, executed, stdout, stderr) {
        ...
        if (executed) {
          if (options.force) {
            options.log({
              message: "Use force to remove live container " + options.container,
              level: 'INFO',
              module: 'mecano/docker/run'
            });
            return docker.exec("rm -f " + options.container + " || true", options, null, function(err) {
              ...
              return docker.exec(cmd, options, null, callback);
            });
          } else {
            options.log({
          ...
          options.log({
            message: "Running container " + options.container,
            level: 'INFO',
            module: 'mecano/docker/run'
          });
          return docker.exec(cmd, options, null, callback);
