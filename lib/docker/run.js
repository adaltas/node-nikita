...
module.exports = function(options, callback) {
  ...
  if (options.name != null) {
    ...
    return docker.exec(" ps -a | grep '" + options.name + "'", options, true, (function(_this) {
      return function(err, executed, stdout, stderr) {
        ...
        if (executed) {
          if (options.force) {
            options.log({
              message: "Use force to remove live container " + options.name,
              level: 'INFO',
              module: 'mecano/docker/run'
            });
            return docker.exec(" rm -f " + options.name + " || true", options, null, function(err) {
              if (err) {
                return callback(err);
              }
