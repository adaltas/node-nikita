...
module.exports = function(options, callback) {
  ...
  return this.write({
    ...
    if ((options.cwd != null) || (options.dockerfile != null)) {
      options.log({
      ...
      return function(err, result) {
        ...
        if (err) {
          ...
          return _this.remove({
            ...
            options.log({
              message: "Building docker image:" + options.image,
              level: 'INFO',
              module: 'mecano/src/docker/build'
            });
            return docker.exec(cmd, options, null, (function(_this) {
              return function(err, executed, stdout, stderr) {
                ...
                if (executed === null) {
                  options.log({
                    message: "new image is identical to previous " + options.image,
                    level: 'INFO',
                    module: 'mecano/src/docker/build'
                  });
                }
                if (executed) {
                  options.log({
                    message: "new image hash  " + container_id_hash,
                    level: 'INFO',
                    module: 'mecano/src/docker/build'
                  });
                }
