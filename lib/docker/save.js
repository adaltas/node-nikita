    ...
    return function(err, executed) {
      ...
      return docker.exec(cmd, options, null, function(err, executed, stdout, stderr) {
        ...
        return ssh2fs.exists(options.ssh, options.destination, function(err, exists) {
          ...
          if (exists) {
            options.log({
              message: "Target saved image already exist :" + options.destination,
              level: 'INFO',
              module: 'mecano/src/docker/save'
            });
            return file.hash(options.ssh, temp_dir_path, 'md5', function(err, value_temp_dir) {
              ...
              return file.hash(options.ssh, options.destination, 'md5', function(err, value_dest) {
                ...
                if (value_temp_dir === value_dest) {
                  options.log({
                    message: "Indetical image (not overwritten):" + options.destination,
                    level: 'INFO',
                    module: 'mecano/src/docker/save'
                  });
                  return _this.remove({
                  ...
                  options.log({
                    message: "Not identical image (overwriting):" + options.destination,
                    level: 'INFO',
                    module: 'mecano/src/docker/save'
                  });
                  _this.copy({
            ...
            options.log({
              message: "Target saved image does not exist :" + options.destination,
              level: 'INFO',
              module: 'mecano/src/docker/save'
            });
            _this.copy({
