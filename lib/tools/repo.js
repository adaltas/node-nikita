...
var fs, glob, misc, path, string;

module.exports = function(options) {
  var keys, remote_files;
  options.log({
    ...
    return glob(options.ssh, "/etc/yum.repos.d/" + options.replace, function(err, files) {
      ...
      remote_files = (function() {
        ...
        for (i = 0, len = files.length; i < len; i++) {
          file = files[i];
          if (file === path.basename(options.source)) {
            continue;
          }
  ...
  this.call({
    "if": function() {
      return options.verify || this.status(-1);
    }
  }, function() {
    ...
    this.call(function(_, callback) {
      ...
      return fs.readFile(options.ssh, options.source, 'utf8', (function(_this) {
