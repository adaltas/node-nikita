...
var EventEmitter, Ftp, child, conditions, curl, diff, each, eco, exec, fs, ldap, ldap_client, mecano, misc, pad, path, request, url, util;
...
mecano = module.exports = {
  chmod: function(goptions, options, callback) {
    ...
    return misc.options(options, function(err, options) {
      ...
      return each(options).parallel(goptions.parallel).on('item', function(options, next) {
        ...
        return misc.file.stat(ssh, options.destination, function(err, stat) {
          if (err) {
            return next(err);
          }
          ...
          return misc.file.chmod(ssh, options.destination, mode, function(err) {
            if (err) {
              return next(err);
            }
            modified++;
    ...
    return misc.options(options, function(err, options) {
      ...
      return each(options).parallel(goptions.parallel).on('item', function(options, next) {
        ...
        return misc.file.stat(ssh, options.destination, function(err, stat) {
          ...
          return misc.file.chown(ssh, options.destination, uid, gid, function(err) {
            if (err) {
              return next()(err);
            }
            modified++;
  ...
  download: function(goptions, options, callback) {
    ...
    return misc.options(options, function(err, options) {
      ...
      return each(options).parallel(goptions.parallel).on('item', function(options, next) {
        ...
        download = function() {
          var cmd, ftp, pass, rs, u, user, ws, _ref1;
          if (typeof options.log === "function") {
      ...
      return each(options).parallel(goptions.parallel).on('item', function(options, next) {
        ...
        return conditions.all(options, next, function() {
          ...
          return each(options.acls).parallel(false).on('item', function(acl, next) {
            var b, client, connect, end, i, parse, save, search, stringify, unbind, _i, _len, _ref1;
            if (acl.before == null) {
  ...
  ldap_index: function(goptions, options, callback) {
    ...
    misc.options(options, function(err, options) {
      ...
      modified = 0;
      return each(options).parallel(goptions.parallel).on('item', function(options, next) {
        var client, connect, end, get, parse, replace, stringifiy, unbind, updated;
        client = null;
        updated = false;
        connect = function() {
          if (options.ldap instanceof ldap_client) {
  ...
  write: function(goptions, options, callback) {
    ...
    misc.options(options, function(err, options) {
      ...
      written = 0;
      return each(options).parallel(goptions.parallel).on('item', function(options, next) {
        var append, backup, between, content, destination, destinationHash, do_changeOwnership, do_changePermissions, do_end, from, modified, readDestination, readSource, render, replacePartial, to, write, writeContent, writeDiff, _ref1;
        modified = false;
        ...
        readSource = function() {
          ...
          if (typeof options.log === "function") {
            options.log("Read source: " + source + (options.local_source ? ' (local)' : void 0));
          }
          ssh = options.local_source ? null : options.ssh;
