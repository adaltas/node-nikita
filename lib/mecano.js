...
mecano = module.exports = {
  ...
  */
  ...
  link: function(options, callback) {
    ...
    sym_exists = function(option, callback) {
      return misc.file.exists(options.ssh, option.destination, function(err, exists) {
        if (!exists) {
    ...
    exec_exists = function(option, callback) {
      return misc.file.exists(options.ssh, option.destination, function(err, exists) {
        ...
        return misc.file.readFile(option.ssh, option.destination, function(err, content) {
          var exec_cmd;
          if (err) {
            return callback(err);
          }
    ...
    exec_create = function(option, callback) {
      var content;
      content = "#!/bin/bash\nexec " + option.source + " $@";
      return misc.file.writeFile(options.ssh, option.destination, content, function(err) {
        if (err) {
          return callback(err);
        }
      ...
      create = function() {
        ...
        return each(dirs).on('item', function(dir, next) {
          ...
          return misc.file.exists(options.ssh, current, function(err, exists) {
            ...
            return misc.file.mkdir(option.ssh, current, option.chmod, function(err) {
              if (err) {
                return next(err);
              }
              dirCreated = true;
  ...
  render: function(options, callback) {
    ...
    return each(options).on('item', function(option, next) {
      ...
      readSource = function() {
        ...
        return misc.file.exists(options.ssh, option.source, function(err, exists) {
          if (!exists) {
            return next(new Error("Invalid source, got " + (JSON.stringify(option.source))));
          }
          return misc.file.readFile(option.ssh, option.source, function(err, content) {
            if (err) {
              return next(err);
            }
  ...
  */
  ...
  write: function(options, callback) {
    ...
    written = 0;
    return each(options).on('item', function(option, next) {
      var destination, destinationHash, readDestination, readSource, render, source, writeContent;
      if (!(option.source || option.content)) {
        return next(new Error('Missing source or content'));
      }
      if (option.source && option.content) {
      ...
      readSource = function() {
        ...
        ssh = option.local_source ? null : option.ssh;
        return misc.file.readFile(ssh, option.source, function(err, content) {
          source = content;
          return readDestination();
        });
      };
      readDestination = function() {
        ...
        return misc.file.exists(option.ssh, option.destination, function(err, exists) {
          if (!exists) {
            return render();
          }
          return misc.file.readFile(option.ssh, option.destination, function(err, content) {
      ...
      writeContent = function() {
        ...
        if (typeof option.destination === 'function') {
          ...
          return misc.file.writeFile(option.ssh, option.destination, source, function(err) {
            if (err) {
              return next(err);
            }
            written++;
