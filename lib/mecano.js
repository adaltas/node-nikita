...
var EventEmitter, child, conditions, each, eco, exec, fs, mecano, misc, open, path, rimraf, util;
      ...
      return each(options).on('item', function(options, next) {
        ...
        writeContent = function() {
          ...
          if (typeof options.destination === 'function') {
            ...
            return misc.file.writeFile(options.ssh, options.destination, source, options, function(err) {
              if (err) {
                return next(err);
              }
              written++;
