...
var conditions, each, eco, exec, fs, mecano, misc, open, path, rimraf, util;
        ...
        cmd = function() {
          ...
          return run.on("exit", function(code) {
            return setTimeout(function() {
              stdouts.push(stds ? stdout : void 0);
              stderrs.push(stds ? stderr : void 0);
  ...
  mkdir: function(options, callback) {
    return misc.options(options, function(err, options) {
      ...
      return each(options).on('item', function(options, next) {
        ...
        if ((options.source == null) && (options.directory != null)) {
        ...
        create = function() {
          var current, dirCreated, dirs, _ref1;
          if ((_ref1 = options.chmod) == null) {
            options.chmod = 0x1ed;
          }
          current = '';
          dirCreated = false;
          ...
          return each(dirs).on('item', function(dir, next) {
            ...
            return misc.file.exists(options.ssh, current, function(err, exists) {
              ...
              return misc.file.mkdir(options.ssh, current, options.chmod, function(err) {
                if (err) {
                  return next(err);
                }
                dirCreated = true;
      ...
      return each(options).on('item', function(options, next) {
        ...
        readSource = function() {
          ...
          return misc.file.exists(options.ssh, options.source, function(err, exists) {
            ...
            return misc.file.readFile(options.ssh, options.source, function(err, content) {
              if (err) {
                return next(err);
              }
  ...
  */
  ...
  write: function(options, callback) {
    return misc.options(options, function(err, options) {
      ...
      written = 0;
      return each(options).on('item', function(options, next) {
        var backup, destination, destinationHash, readDestination, readSource, render, source, writeContent;
        if (!(options.source || options.content)) {
          return next(new Error('Missing source or content'));
        }
        if (options.source && options.content) {
        ...
        writeContent = function() {
          if (destinationHash === misc.string.hash(source)) {
            return next();
          }
          if (typeof options.destination === 'function') {
            ...
            return misc.file.writeFile(options.ssh, options.destination, source, function(err) {
              if (err) {
                return next(err);
              }
              written++;
        ...
        backup = function() {
          ...
          return misc.file.writeFile(options.ssh, backup, source, function(err) {
            if (err) {
              return next(err);
            }
            return next();
  ...
  */
  ...
  service: function(options, callback) {
    return misc.options(options, function(err, options) {
      ...
      serviced = 0;
      return each(options).on('item', function(options, next) {
        var chkname, finish, install, installed, modified, pkgname, srvname, start, started, startup, startuped, updates;
        if (!options.name) {
        ...
        started = function() {
          ...
          return mecano.execute({
            ...
            start = options.start != null ? options.start : options.stop;
            if (!started && start) {
              start();
            }
            if (started && !start) {
