...
module.exports = function(options) {
  var j, len, ref, ssh, target, targetHash, targetStat, w;
  options.log({
  ...
  this.call(function(_, callback) { // read source
    ...
    return fs.exists(sshOrLocal, source, function(err, exists) {
      if (err) {
        return callback(err);
      }
      if (!exists) {
      ...
      return fs.readFile(sshOrLocal, source, 'utf8', function(err, src) {
        if (err) {
          return callback(err);
        }
        options.content = src;
  ...
  this.call(function(_, callback) { // read target
    ...
    exists = function() {
      ...
      return fs.lstat(ssh, options.target, function(err, stat) {
        ...
        if (stat.isDirectory()) {
          ...
          return fs.unlink(ssh, options.target, function(err, stat) {
            if (err) {
              return callback(err);
            }
    ...
    do_read = function() {
      ...
      return fs.readFile(ssh, options.target, 'utf8', function(err, dest) {
        if (err) {
          return callback(err);
        }
  ...
  this.call(function(_, callback) { // file
    ...
    return uid_gid(ssh, options, function(err) {
      if (err) {
        return callback(err);
      }
      if (!targetStat) {
      ...
      return fs.writeFile(ssh, options.target, options.content, options, function(err) {
        if (err) {
          return callback(err);
        }
        options.log({
