...
module.exports = function(options) {
  ...
  this.call(function(_, callback) { // file
    ...
    return uid_gid(ssh, options, function(err) {
      ...
      target = !options.sudo ? options.target : target = `/tmp/nikita_${string.hash(options.target)}`;
      this.call({
        if: options.sudo
      }, function() {
        return options.log({
          message: `Temporary Upload: ${target}`,
          level: 'INFO',
          module: 'nikita/lib/file'
        });
      });
      this.call(function(_, callback) {
        ...
        return fs.writeFile(ssh, target, options.content, function(err) {
          if (err) {
            return callback(err);
          }
          options.log({
