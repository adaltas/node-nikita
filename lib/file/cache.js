...
module.exports = function(options, callback) {
  ...
  if (options.md5 != null) {
    ...
    if ((ref2 = typeof options.sha256) !== 'string' && ref2 !== 'boolean') {
      return callback(Error(`Invalid SHA-1 Hash:${options.sha256}`));
    }
    algo = 'sha256';
    hash = options.sha256;
  } else {
  ...
  this.call(function(_, callback) {
    ...
    return file.hash(ssh, options.source, algo, function(err, value) {
      if (err) {
        return callback(err);
      }
      options.log({
  ...
  this.call({
    ...
    return ssh2fs.exists(ssh, options.target, (err, exists) => {
      if (err) {
        return callback(err);
      }
      if (exists) {
        ...
        if (options.force) {
          ...
          return file.hash(ssh, options.target, algo, function(err, c_hash) {
            if (err) {
              return callback(err);
            }
            ...
            return ssh2fs.unlink(ssh, options.target, function(err) {
              if (err) {
                return callback(err);
              }
              return callback(null, true);
  ...
  if (ref3 = u.protocol, indexOf.call(protocols_http, ref3) >= 0) {
    ...
    this.system.mkdir({
      ssh: options.cache_local ? null : ssh,
