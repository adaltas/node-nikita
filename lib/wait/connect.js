...
module.exports = function(options, callback) {
  ...
  options.log({
    ...
    level: 'DEBUG',
    module: 'mecano/wait/connect'
  });
  return each(servers).parallel(true).call((function(_this) {
    return function(server, next) {
      var child, clear, count, rand, randfile, timedout;
      count = 0;
      rand = Date.now() + inc++;
      ...
      if (options.timeout && options.timeout > 0) {
        ...
        clear = setTimeout(function() {
          timedout = true;
          return _this.child().remove({
            destination: randfile
          });
        }, options.timeout);
      }
      options.log({
        message: "Start wait for " + server.host + ":" + server.port,
        level: 'DEBUG',
        module: 'mecano/wait/connect'
      });
      child = _this.child();
      return child.execute({
        ...
        if (!err) {
          if ((ref4 = options.ready) != null) {
            ref4.call(_this, server, executed);
          }
        }
        if (executed) {
          modified = true;
        ...
        options.log({
          message: "Finish wait for " + server.host + " " + server.port,
          level: 'INFO',
          module: 'mecano/wait/connect'
        });
        if (!err) {
