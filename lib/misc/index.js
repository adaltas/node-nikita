...
var Stream, buffer, connect, crypto, each, exec, fs, glob, ini, misc, path, rimraf, ssh2fs, tilde, util,
  __slice = [].slice;
...
misc = module.exports = {
  ...
  file: {
    ...
    cmpmod: function() {
      ...
      return (_ref = misc.mode.compare).call.apply(_ref, [this].concat(__slice.call(modes)));
    },
      ...
      return ssh2fs.stat(ssh, file, function(err, stat) {
        ...
        if (stat.isFile()) {
          ...
          compute = function(files) {
            files.sort();
            return each(files).on('item', function(item, next) {
          ...
          if (ssh) {
            return exec(ssh, "find " + file + " -name '**'", function(err, stdout) {
              var files;
              if (err) {
                return callback(err);
              }
            ...
            return glob("" + file + "/**", function(err, files) {
              if (err) {
                return callback(err);
              }
              return compute(files);
  ...
  ini: {
    ...
    *   `comment`   Default to ";"
    ...
    stringify_square_then_curly: function(content, depth, options) {
      var element, i, indent, isArray, isBoolean, isNull, isObj, k, out, outa, prefix, v, _i, _j, _len;
      if (depth == null) {
        depth = 0;
      }
      if (options == null) {
    ...
    stringify_multi_brackets: function(content, depth, options) {
      var i, indent, isBoolean, isNull, isObj, k, out, prefix, v, _i;
      if (depth == null) {
        depth = 0;
      }
      if (options == null) {
