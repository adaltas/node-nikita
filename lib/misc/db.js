...
module.exports.cmd = function() {
  ...
  switch (options.engine) {
    case 'mysql':
      return ["mysql", "-h " + options.host, "-u " + (options.admin_username || options.username), "-p " + (options.admin_password || options.password), cmd ? "\"" + cmd + "\"" : void 0].join(' ');
    case 'postgres':
      return ["PGPASSWORD=" + (options.admin_password || options.password), "psql", "-h " + options.host, "-U " + (options.admin_username || options.username), options.database ? "-d " + options.database : void 0, cmd ? "-tAc \"" + cmd + "\"" : void 0].join(' ');
    default:
