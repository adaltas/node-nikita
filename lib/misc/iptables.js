// Generated by CoffeeScript 1.7.1
var iptables, jsesc, misc;

misc = require('./index');

jsesc = require('jsesc');

module.exports = iptables = {
  add_properties: ['-p', '-s', '-d', '-g', '-i', '-o', '-f', 'tcp|--dport', 'udp|--dport'],
  modify_properties: ['-c', 'state|--state', 'comment|--comment', 'tcp|--source-port', 'tcp|--sport', 'tcp|--destination-port', 'tcp|--dport', 'tcp|--tcp-flags', 'tcp|--syn', 'tcp|--tcp-option', 'udp|--source-port', 'udp|--sport', 'udp|--destination-port', 'udp|--dport'],
  commands: {
    '-A': ['chain'],
    '-D': ['chain'],
    '-I': ['chain'],
    '-R': ['chain'],
    '-P': ['chain', 'target'],
    '-L': true,
    '-S': true,
    '-F': true,
    '-Z': true,
    '-N': true,
    '-X': true,
    '-E': true
  },
  parameters: ['-p', '-s', '-d', '-j', '-g', '-i', '-o', '-f', '-c'],
  parameters_inverted: {
    '--protocol': '-p',
    '--source': '-s',
    '--destination': '-d',
    '--jump': '-j',
    '--goto': '-g',
    '--in-interface': '-i',
    '--out-interface': '-o',
    '--fragment': '-f',
    '--set-counters': '-c'
  },
  protocols: {
    tcp: ['--source-port', '--sport', '--destination-port', '--dport', '--tcp-flags', '--syn', '--tcp-option'],
    udp: ['--source-port', '--sport', '--destination-port', '--dport'],
    udplite: [],
    icmp: [],
    esp: [],
    ah: [],
    sctp: [],
    all: []
  },
  modules: {
    state: ['--state'],
    comment: ['--comment']
  },
  cmd_args: function(cmd, rule) {
    var arg, k, match, module, v;
    for (k in rule) {
      v = rule[k];
      if (['chain', 'rulenum'].indexOf(k) !== -1) {
        continue;
      }
      if (v == null) {
        continue;
      }
      if (match = /^([\w]+)\|([-\w]+)$/.exec(k)) {
        module = match[1];
        arg = match[2];
        cmd += " -m " + module;
        cmd += " " + arg + " " + v;
      } else {
        cmd += " " + k + " " + v;
      }
    }
    return cmd;
  },
  cmd_modify: function(rule) {
    if (rule.rulenum == null) {
      rule.rulenum = 1;
    }
    return iptables.cmd_args("iptables -R " + rule.chain + " " + rule.rulenum, rule);
  },
  cmd_add: function(rule) {
    if (rule.rulenum == null) {
      rule.rulenum = 1;
    }
    return iptables.cmd_args("iptables -I " + rule.chain + " " + rule.rulenum, rule);
  },
  cmd: function(oldrules, newrules) {
    var add_properties, cmds, create, i, k, newrule, oldrule, rulenum, v, _i, _j, _k, _l, _len, _len1, _len2, _len3;
    cmds = [];
    for (_i = 0, _len = newrules.length; _i < _len; _i++) {
      newrule = newrules[_i];
      if (newrule.rulenum != null) {
        break;
      }
      if (newrule.after) {
        rulenum = 0;
        for (i = _j = 0, _len1 = oldrules.length; _j < _len1; i = ++_j) {
          oldrule = oldrules[i];
          if (oldrule['-A'] !== newrule.chain) {
            continue;
          }
          rulenum++;
          if (misc.object.equals(newrule.after, oldrule, Object.keys(newrule.after))) {
            newrule.rulenum = rulenum + 1;
            break;
          }
        }
        delete newrule.after;
      }
      if (newrule.before) {
        rulenum = 0;
        for (i = _k = 0, _len2 = oldrules.length; _k < _len2; i = ++_k) {
          oldrule = oldrules[i];
          if (oldrule['-A'] !== newrule.chain) {
            continue;
          }
          rulenum++;
          if (misc.object.equals(newrule.before, oldrule, Object.keys(newrule.before))) {
            newrule.rulenum = rulenum;
            break;
          }
        }
        delete newrule.before;
      }
      create = true;
      add_properties = misc.array.intersect(iptables.add_properties, Object.keys(newrule));
      for (_l = 0, _len3 = oldrules.length; _l < _len3; _l++) {
        oldrule = oldrules[_l];
        if (misc.object.equals(newrule, oldrule, add_properties)) {
          create = false;
          if (!misc.object.equals(newrule, oldrule, iptables.modify_properties)) {
            for (k in oldrule) {
              v = oldrule[k];
              if (iptables.commands[k]) {
                oldrule[k] = null;
              }
            }
            cmds.push(iptables.cmd_modify(misc.merge(oldrule, newrule)));
          }
        }
      }
      if (create) {
        cmds.push(iptables.cmd_add(newrule));
      }
    }
    return cmds;
  },
  normalize: function(rules) {
    var k, mk, mv, mvs, newrule, newrules, nk, oldrules, protocol, rule, v, _i, _j, _k, _len, _len1, _len2, _ref, _ref1;
    oldrules = Array.isArray(rules) ? rules : [rules];
    newrules = [];
    for (_i = 0, _len = oldrules.length; _i < _len; _i++) {
      rule = oldrules[_i];
      rule = misc.merge({}, rule);
      newrule = {};
      for (k in rule) {
        v = rule[k];
        if (typeof v === 'number') {
          v = rule[k] = "" + v;
        }
        if (k === 'chain' || k === 'rulenum') {
          nk = k;
        } else if (k.slice(0, 2) === '--') {
          nk = iptables.parameters_inverted[k];
        } else if (k[0] !== '-') {
          nk = iptables.parameters_inverted["--" + k];
        } else if (iptables.parameters.indexOf(k) !== -1) {
          nk = k;
        }
        if (nk) {
          newrule[nk] = v;
          rule[k] = null;
        }
      }
      if (protocol = newrule['-p']) {
        _ref = iptables.protocols[protocol];
        for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
          k = _ref[_j];
          if (rule[k]) {
            newrule["" + protocol + "|" + k] = rule[k];
            rule[k] = null;
          } else if (rule[k.slice(2)]) {
            newrule["" + protocol + "|" + k] = rule[k.slice(2)];
            rule[k.slice(2)] = null;
          }
        }
      }
      for (k in rule) {
        v = rule[k];
        if (!v) {
          continue;
        }
        if (k === 'after' || k === 'before') {
          newrule[k] = iptables.normalize(v);
          continue;
        }
        if (k.slice(0, 2) !== '--') {
          k = "--" + k;
        }
        _ref1 = iptables.modules;
        for (mk in _ref1) {
          mvs = _ref1[mk];
          for (_k = 0, _len2 = mvs.length; _k < _len2; _k++) {
            mv = mvs[_k];
            if (k === mv) {
              newrule["" + mk + "|" + k] = v;
              rule[k] = null;
            }
          }
        }
      }
      for (k in newrule) {
        v = newrule[k];
        if (/\-/.test(v)) {
          v = v.replace('-', '');
        }
        if (k === 'comment|--comment') {
          v = jsesc(v, {
            quotes: 'double',
            wrap: true
          });
        }
        newrule[k] = v;
      }
      newrules.push(newrule);
    }
    if (Array.isArray(rules)) {
      return newrules;
    } else {
      return newrules[0];
    }
  },

  /*
  Parse the result of `iptables -S`
   */
  parse: function(stdout) {
    var char, command, command_index, forceflush, i, j, key, line, module, newarg, rule, rules, v, value, _i, _j, _len, _len1, _ref, _ref1;
    rules = [];
    command = null;
    command_index = 1;
    _ref = stdout.split(/\r\n|[\n\r\u0085\u2028\u2029]/g);
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      line = _ref[_i];
      if (line.length === 0) {
        continue;
      }
      command_index++;
      rule = {};
      i = 0;
      key = '';
      value = '';
      module = null;
      while (i <= line.length) {
        char = line[i];
        forceflush = i === line.length;
        newarg = (i === 0 && char === '-') || line.slice(i - 1, +i + 1 || 9e9) === ' -';
        if (newarg || forceflush) {
          if (value) {
            value = value.trim();
            if (key === '-m') {
              module = value;
            } else {
              if (module) {
                key = "" + module + "|" + key;
              }
              rule[key] = value;
            }
            if (iptables.commands[key]) {
              if (key !== command) {
                command = key;
                command_index = 1;
              }
              rule.rulenum = command_index;
              if (Array.isArray(iptables.commands[key])) {
                _ref1 = value.split(' ');
                for (j = _j = 0, _len1 = _ref1.length; _j < _len1; j = ++_j) {
                  v = _ref1[j];
                  rule[iptables.commands[key][j]] = v;
                }
              }
            }
            key = '';
            value = '';
            if (forceflush) {
              break;
            }
          }
          key += char;
          while ((char = line[++i]) !== ' ') {
            key += char;
          }
          if (iptables.parameters.indexOf(key) !== -1) {
            module = null;
          }
          continue;
        }
        if (char === '"') {
          while ((char = line[++i]) !== '"') {
            value += char;
          }
          i++;
          continue;
        }
        while ((char = line[++i]) !== '-' && i < line.length) {
          value += char;
        }
      }
      rules.push(rule);
    }
    return rules;
  }
};
