...
var Stream, buffer, connect, crypto, each, exec, fs, ini, misc, path, rimraf, util,
  __slice = [].slice;
  ...
  *   2 if pidfile exists but match no process
  ...
  kadmin: function(options, cmd) {
    if (options.kadmin_principal) {
      return "kadmin -p " + options.kadmin_principal + " -s " + options.kadmin_server + " -w " + options.kadmin_password + " -q '" + cmd + "'";
    } else {
      return "kadmin.local -q '" + cmd + "'";
    }
  },
  ini: {
    safe: function(val) {
  ...
  args: function(args, default_goptions) {
    if (default_goptions == null) {
      default_goptions = {};
    }
    ...
    if (args[0] == null) {
      args[0] = misc.merge(default_goptions, {
        parallel: true
      });
    }
    return args;
  ...
  *   `options`       Sanitized options.
  ...
  options: function(options, callback) {
    ...
    return each(options).on('item', function(options, next) {
      var connection, el, gid, i, mode, uid, v, _i, _j, _len, _len1, _ref, _ref1;
