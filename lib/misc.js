...
var EventEmitter, ProxyStream, Stream, crypto, each, exec, fs, misc, path, ssh2, util, _ref;
...
module.exports = misc = {
  exec: function(options, callback) {
    var child, cmdOptions, connect, connection, run;
    if (options.ssh) {
      ...
      connect = function() {
        ...
        if ((_ref1 = (_base = options.ssh).username) == null) {
          _base.username = process.env['USER'];
        }
        if ((_ref2 = (_base1 = options.ssh).port) == null) {
          _base1.port = 22;
        }
        if (!options.ssh.password && !options.ssh.privateKey) {
          options.ssh.privateKey = fs.readFileSync("" + process.env['HOME'] + "/.ssh/id_rsa");
        }
        return connection.connect(options.ssh);
      };
      run = function() {
        var stderr, stdout;
        stdout = stderr = '';
