...
var ProxyStream, ST_MODE, Stat, Stream, buffer, connect, crypto, each, exec, fs, misc, path, rimraf, util, _ref;
    ...
    writeFile: function(ssh, path, content, options, callback) {
      ...
      if (!ssh) {
        ...
        chown = function() {
          ...
          return fs.chown(path, options.uid, options.gid, function(err) {
            if (err) {
              return callback(err);
            }
            return chmod();
        ...
        chmod = function() {
          ...
          return fs.chmod(path, options.mode, function(err) {
            return finish(err);
          });
        };
        finish = function(err) {
    ...
    passwd: function(ssh, username, callback) {
      if (arguments.length === 3) {
        ...
        return misc.ssh.passwd(ssh, function(err, users) {
          ...
          user = users[username];
          if (!user) {
            return new Error("User " + username + " does not exists");
          }
      ...
      return misc.file.readFile(ssh, '/etc/passwd', function(err, lines) {
        var info, line, passwd, _i, _len, _ref1;
    ...
    group: function(ssh, group, callback) {
      if (arguments.length === 3) {
        ...
        return misc.ssh.group(ssh, function(err, groups) {
          ...
          group = groups[group];
          if (!group) {
            return new Error("Group " + group + " does not exists");
          }
      ...
      group = null;
      if (ssh.group) {
        return callback(null, ssh.group);
      }
      return misc.file.readFile(ssh, '/etc/group', function(err, lines) {
        var info, line, _i, _len, _ref1;
