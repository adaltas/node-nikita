...
// ## Hooks
var handler, on_action, schema, url, utils;

on_action = function({config}) {
  return config.method != null ? config.method : config.method = config.request;
};
...
\`negociate\` option.`
    ...
    request: {
      ...
      description: `Alias for \`method\` respecting the curl naming.`
    },
    target: {
      type: 'string',
...
${[
    ...
    err = error;
    if (err.exit_code === 3) {
      throw Error("Required Dependencies: curl is required to perform HTTP requests");
    }
    throw err;
  }
  await this.fs.chmod({
