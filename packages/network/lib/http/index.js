  ...
  handler: async function({config}) {
    ...
    try {
      const {stdout} = await this.execute({
        command: dedent`
          ${!config.principal ? '' : ['echo', config.password, '|', 'kinit', config.principal, '>/dev/null'].join(' ')}
          ...
          ${[
            ...
            !config.cacert && config.url.startsWith('https:') ? '--insecure' : void 0,
            config.cacert ? '--cacert #{config.cacert}' : void 0,
            config.negotiate ? '--negotiate -u:' : void 0,
            config.location ? '--location' : void 0,
            ...Object.keys(config.http_headers).map(header => `--header ${esa(header+": "+config.http_headers[header])}`),
      ...
      for (const line of utils.string.lines(stdout)) {
        if (output.body.length === 0 && /^HTTP\/[\d.]+ \d+/.test(line)) {
          ...
          output.headers = {};
          const [http_version, status_code, ...status_message] = line.split(' ');
