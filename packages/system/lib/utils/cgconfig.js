    ...
    string.lines(str).forEach(function (line) {
      ...
      if (!current_mount && !current_group && !current_default) {
        ...
        if (/^(default)\s{$/.test(line)) {
          // start of a special group object named default
          current_group = true;
          current_group_name = "";
          current_group_section = {};
          list_of_group_sections[current_group_name] ??= {};
        ...
        if (current_group) {
          ...
          if (/^(\s*)?}$/.test(line)) {
            ...
            const match =
              /^\s*(cpuset|cpu|cpuacct|blkio|memory|devices|freezer|net_cls|perf_event|net_prio|hugetlb|pids|rdma)\s{$/.exec(
                line,
              );
            if (!current_group_perm && !current_group_controller) {
              ...
              current_group_section["perm"][current_group_section_perm_name][
                type.trim()
              ] = value.trim();
              list_of_group_sections[`${current_group_name}`]["perm"][
                current_group_section_perm_name
              ...
              if (match_admin) {
                // admin or task declaration
                const [, name] = match_admin; //the name is either admin or task
