    ...
    try {
      ...
      return {
        ...
        files: utils.string
          .lines(stdout)
          .filter(
            (line, i) => !config.extended || i !== 0, // First line in extended mode
          )
          .filter(
            (line) => line !== "", // Empty lines
          )
          .map(function (line) {
            if (!config.extended) {
              return {
  ...
  hooks: {
    on_action: function ({ config }) {
      if (config.withFileTypes != null) {
        return config.extended != null ?
            config.extended
          : (config.extended = config.withFileTypes);
      }
    },
  },
  metadata: {
