...
const errors = {
  ...
  NIKITA_FS_CRS_TARGET_ENOENT: ({error, config}) =>
    utils.error('NIKITA_FS_CRS_TARGET_ENOENT', ['fail to read a file because it does not exist,', !config.target_tmp ? `location is ${JSON.stringify(config.target)}.` : `location is ${JSON.stringify(config.target_tmp)} (temporary file, target is ${JSON.stringify(config.target)}).`], {
      errno: error.errno,
  ...
  NIKITA_FS_CRS_TARGET_EISDIR: ({error, config}) =>
    utils.error('NIKITA_FS_CRS_TARGET_EISDIR', ['fail to read a file because it is a directory,', !config.target_tmp ? `location is ${JSON.stringify(config.target)}.` : `location is ${JSON.stringify(config.target_tmp)} (temporary file, target is ${JSON.stringify(config.target)}).`], {
      errno: error.errno,
      syscall: error.syscall,
  ...
  NIKITA_FS_CRS_TARGET_EACCES: ({error, config}) =>
    utils.error('NIKITA_FS_CRS_TARGET_EACCES', ['fail to read a file because permission was denied,', !config.target_tmp ? `location is ${JSON.stringify(config.target)}.` : `location is ${JSON.stringify(config.target_tmp)} (temporary file, target is ${JSON.stringify(config.target)}).`], {
      errno: error.errno,
      syscall: error.syscall,
      path: config.target_tmp || config.target // Native Node.js api doesn't provide path
    })
...
export default {
  handler: async function({
    ...
    return new Promise(async function(resolve, reject) {
      const buffers = [];
      const rs = await fs.createReadStream(ssh, config.target_tmp || config.target);
