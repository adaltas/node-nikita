...
export default {
  handler: async function ({ config, tools: { $status, log, path } }) {
      ...
      async function () {
        ...
        for (const source of files) {
          await (async (source) => {
            ...
            let uid = config.uid;
            if (config.preserve && uid == null) {
              uid = stats.uid;
            }
            let gid = config.gid;
            if (config.preserve && gid == null) {
              gid = stats.gid;
            }
            let mode = config.mode;
            if (config.preserve && mode == null) {
              mode = stats.mode;
            }
    ...
    if (config.preserve && config.uid == null) {
      // File ownership and permissions
      config.uid = source_stats.uid;
    }
    if (config.preserve && config.gid == null) {
      config.gid = source_stats.gid;
    }
    if (config.preserve && config.mode == null) {
