...
const errors = {
  ...
  NIKITA_FS_ASSERT_FILE_EXISTS: function ({ config }) {
    return utils.error(
      ...
      ["file exists,", `location is ${JSON.stringify(config.target)}.`],
      {
        target: config.target,
        message: config.error,
      },
  ...
  NIKITA_FS_ASSERT_FILETYPE_INVALID: function ({ config, expect, stats }) {
    return utils.error(
      "NIKITA_FS_ASSERT_FILETYPE_INVALID",
      [
        "filetype is invalid,",
...
export default {
  handler: async function ({ config }) {
    ...
    if (
      config.content == null &&
      !(config.md5 || config.sha1 || config.sha256 || config.mode?.length)
    ) {
    ...
    if (config.filetype?.length) {
      ...
      if (
        config.filetype.includes(fs.constants.S_IFREG) &&
      ...
      if (
        config.filetype.includes(fs.constants.S_IFDIR) &&
      ...
      if (
        config.filetype.includes(fs.constants.S_IFCHR) &&
        !utils.stats.isCharacterDevice(stats.mode)
      ) {
        throw errors.NIKITA_FS_ASSERT_FILETYPE_INVALID({
          config: config,
      ...
      if (
        config.filetype.includes(fs.constants.S_IFBLK) &&
        !utils.stats.isBlockDevice(stats.mode)
      ) {
        throw errors.NIKITA_FS_ASSERT_FILETYPE_INVALID({
          config: config,
          expect: "Block Device",
      ...
      if (
        config.filetype.includes(fs.constants.S_IFIFO) &&
        !utils.stats.isFIFO(stats.mode)
      ) {
        throw errors.NIKITA_FS_ASSERT_FILETYPE_INVALID({
          config: config,
          expect: "FIFO",
      ...
      if (
        config.filetype.includes(fs.constants.S_IFLNK) &&
        !utils.stats.isSymbolicLink(stats.mode)
      ) {
        throw errors.NIKITA_FS_ASSERT_FILETYPE_INVALID({
          config: config,
      ...
      if (
        config.filetype.includes(fs.constants.S_IFSOCK) &&
        !utils.stats.isSocket(stats.mode)
      ) {
        throw errors.NIKITA_FS_ASSERT_FILETYPE_INVALID({
          config: config,
          expect: "Socket",
    ...
    if (
      config.content != null &&
      (typeof config.content === "string" || Buffer.isBuffer(config.content))
    ) {
