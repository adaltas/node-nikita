...
const equals = function(obj1, obj2, keys) {
...
const iptables = {
  ...
  command: function(oldrules, newrules) {
    const commands = [];
    const new_chains = [];
    const old_chains = oldrules.map(function(oldrule) {
    ...
    for (const newrule of newrules) {
      ...
      const add_properties = utils.array.intersect(iptables.add_properties, Object.keys(newrule));
      for (const oldrule of oldrules) {
    ...
    for (const oldrule of oldrules) {
      ...
      for (const key in oldrule) {
        ...
        if (key === 'chain' || key === 'rulenum' || key === 'command') {
          ...
          nkey = key;
        } else if (key.slice(0, 2) === '--' && iptables.parameters.indexOf(key) >= 0) {
          // nkey = iptables.parameters_inverted[k]
          nkey = key;
        } else if (key[0] !== '-' && iptables.parameters.indexOf(`--${key}`) >= 0) {
          // nkey = iptables.parameters_inverted["--#{key}"]
          nkey = `--${key}`;
        // else if iptables.parameters.indexOf(key) isnt -1
        } else if (iptables.parameters_inverted[key]) {
          nkey = iptables.parameters_inverted[key];
        }
        // nkey = key
      ...
      if (protocol != null) {
        for (const key of iptables.protocols[protocol]) {
          if (oldrule[key]) {
      ...
      for (let key in oldrule) {
        ...
        if (key === 'after' || key === 'before') {
          newrule[key] = iptables.normalize(value, false);
          continue;
        }
        if (key.slice(0, 2) !== '--') {
          key = `--${key}`;
        }
        for (const mk in iptables.modules) {
          const mvs = iptables.modules[mk];
          for (const mv of mvs) {
            if (key === mv) {
  ...
  parse: function(stdout) {
    ...
    for (const line of utils.string.lines(stdout)) {
      ...
      while (i <= line.length) {
        ...
        const newarg = (i === 0 && char === '-') || line.slice((i - 1), +i + 1 || 9e9) === ' -';
            ...
            if (iptables.commands_arguments[key]) {
              ...
              if (Array.isArray(iptables.commands_arguments[key])) {
                ...
                for (const k in valueSplit) {
                  rule[iptables.commands_arguments[key][k]] = valueSplit[k];
                }
