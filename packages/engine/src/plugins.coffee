...
normalize_hook = (event, hook) ->
  ...
  hook.map (hook) ->
    if typeof hook is 'function'
      ...
      throw utils.error 'PLUGINS_HOOK_INVALID_HANDLER', [
...
module.exports = ({action, chain, parent, plugins = []} = {}) ->
  ...
  obj =
    ...
    register: (plugin) ->
      throw utils.error('PLUGINS_REGISTER_INVALID_ARGUMENT', [
    ...
    get: ({event, hooks = [], sort = true}) ->
      hooks = [
        ...
        ...utils.array.flatten( for plugin in store
    ...
    hook: ({args = [], handler, hooks = [], event, silent})->
      if arguments.length isnt 1
        ...
        throw utils.error 'PLUGINS_INVALID_ARGUMENT_EVENT', [
...
errors =
  PLUGINS_HOOK_AFTER_INVALID: ({event, module, after}) ->
    throw utils.error 'PLUGINS_HOOK_AFTER_INVALID', [
  ...
  PLUGINS_HOOK_BEFORE_INVALID: ({event, module, before}) ->
    throw utils.error 'PLUGINS_HOOK_BEFORE_INVALID', [
      "the hook #{JSON.stringify event}"
      "in plugin #{JSON.stringify module}" if module
