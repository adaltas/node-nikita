...
// ## Hook
var constants, error, errors, handler, on_action, schema, schema_output;
  ...
  properties: {
    'stats': {
      ...
      properties: {
        'mode': {
          type: 'integer',
          description: `A bit-field describing the file type and mode.`
        },
        'uid': {
          type: 'integer',
...
errors = {
  NIKITA_FS_STAT_TARGET_ENOENT: function({config, err}) {
    return error('NIKITA_FS_STAT_TARGET_ENOENT', ['failed to stat the target, no file exists for target,', `got ${JSON.stringify(config.target)}`], {
