...
var array, error, errors, is_object, is_object_literal, merge, normalize_hook, toposort;
...
normalize_hook = function(event, hook) {
  ...
  return hook.map(function(hook) {
    if (typeof hook === 'function') {
      ...
      throw error('PLUGINS_HOOK_INVALID_HANDLER', ['no hook handler function could be found,', 'a hook must be defined as a function', 'or as an object with an handler property', `got ${JSON.stringify(hook)}.`]);
    }
    hook.event = event;
  ...
  obj = {
    ...
    register: function(plugin) {
      ...
      if (!is_object_literal(plugin)) {
        throw error('PLUGINS_REGISTER_INVALID_ARGUMENT', ['a plugin must consist of keys representing the hook module name', 'associated with function implementing the hook,', `got ${plugin}.`]);
      }
    ...
    hook: async function({args = [], handler, hooks = [], event, silent}) {
      ...
      if (arguments.length !== 1) {
        ...
        throw error('PLUGINS_INVALID_ARGUMENT_PROPERTIES', ['function hook expect argument to be a literal object', 'with the event, args, hooks and handler properties,', `got ${JSON.stringify(arguments[0])} arguments.`]);
      } else if (typeof event !== 'string') {
        throw error('PLUGINS_INVALID_ARGUMENT_EVENT', ['function hook expect a event properties in its first argument,', `got ${JSON.stringify(arguments[0])} argument.`]);
      }
...
errors = {
  PLUGINS_HOOK_AFTER_INVALID: function({event, module, after}) {
    throw error('PLUGINS_HOOK_AFTER_INVALID', [`the hook ${JSON.stringify(event)}`, module ? `in plugin ${JSON.stringify(module)}` : void 0, 'references an after dependency', `in plugin ${JSON.stringify(after)} which does not exists`]);
  },
  PLUGINS_HOOK_BEFORE_INVALID: function({event, module, before}) {
    throw error('PLUGINS_HOOK_BEFORE_INVALID', [`the hook ${JSON.stringify(event)}`, module ? `in plugin ${JSON.stringify(module)}` : void 0, 'references a before dependency', `in plugin ${JSON.stringify(before)} which does not exists`]);
  }
};
