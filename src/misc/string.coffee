...
module.exports =
    ...
    render: (options) ->
      options.log message: "Rendering with #{options.engine}", level: 'DEBUG', module: 'nikita/lib/write'
      try
    ...
    replace_partial: (options) ->
      ...
      options.log message: "Replacing sections of the file", level: 'DEBUG', module: 'nikita/lib/misc/string'
      for opts in options.write
        if opts.match
          opts.match ?= opts.replace
          options.log message: "Convert match string to regexp", level: 'DEBUG', module: 'nikita/lib/misc/string' if typeof opts.match is 'string'
          ...
          if opts.match.test options.content
            ...
            options.log message: "Match existing partial", level: 'INFO', module: 'nikita/lib/misc/string'
            ...
            if opts.place_before instanceof RegExp
              options.log message: "Replace with match and place_before regexp", level: 'DEBUG', module: 'nikita/lib/misc/string'
              posoffset = 0
              orgContent = options.content
              while (res = opts.place_before.exec orgContent) isnt null
                options.log message: "Before regexp found a match", level: 'INFO', module: 'nikita/lib/misc/string'
              ...
              options.log message: "Forgot how we could get there, test shall say it all", level: 'DEBUG', module: 'nikita/lib/misc/string'
            ...
            if typeof opts.append is "string"
              options.log message: "Convert append string to regexp", level: 'DEBUG', module: 'nikita/lib/misc/string'
              opts.append = new RegExp "^.*#{quote opts.append}.*$", 'mg'
            if opts.append instanceof RegExp
              options.log message: "Replace with match and append regexp", level: 'DEBUG', module: 'nikita/lib/misc/string'
              posoffset = 0
              orgContent = options.content
              while (res = opts.append.exec orgContent) isnt null
                options.log message: "Append regexp found a match", level: 'INFO', module: 'nikita/lib/misc/string'
                pos = posoffset + res.index + res[0].length
          ...
          options.log message: "Before is true, need to explain how we could get here", level: 'INFO', module: 'nikita/lib/misc/string'
          ...
          if opts.from and opts.to
            ...
            if from? and not to?
              options.log message: "Found 'from' but missing 'to', skip writing", level: 'WARN', module: 'nikita/lib/misc/string'
            else if not from? and to?
              options.log message: "Missing 'from' but found 'to', skip writing", level: 'WARN', module: 'nikita/lib/misc/string'
            else if not from? and not to?
              if opts.append
                options.content += '\n' + opts.from + '\n' + opts.replace+ '\n' + opts.to
              else
                options.log message: "Missing 'from' and 'to' without append, skip writing", level: 'WARN', module: 'nikita/lib/misc/string'
            else
            ...
            if from?
              ...
              options.log message: "Missing 'from', skip writing", level: 'WARN', module: 'nikita/lib/misc/string'
